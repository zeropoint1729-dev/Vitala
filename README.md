...
- **In-app language switcher**: a language card on the Tips screen (English / বাংলা pills) calling `AppCompatDelegate.setApplicationLocales()` — the official per-app language API. Persists automatically via AppCompat's `autoStoreLocales` service (registered in the manifest, since `MainActivity` is a plain `ComponentActivity`, not `AppCompatActivity`) and also shows up in Android 13+'s own Settings → Apps → Vitala → Language screen. Switching triggers an activity recreation, same as any locale/config change.

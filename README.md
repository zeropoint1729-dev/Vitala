...
- **Disease library is now fully bilingual and covers more of South Asia**: `assets/diseases.json` (English) and `assets/diseases_bn.json` (Bengali) each have 24 entries — the original 14 plus 10 new ones more associated with the wider South Asian region than Bangladesh specifically (malaria, rabies, snakebite envenoming, leprosy, kala-azar, Japanese encephalitis, chikungunya, hepatitis A, rheumatic heart disease, COPD). IDs match exactly between the two files. `DiseaseRepository.syncLanguage(context)` picks the right file based on the app's current language (`util/LocaleUtils`) and reloads the library via `DiseaseDao.replaceAll` — called from `MainActivity.onCreate()` (not `Application.onCreate()`, since only the Activity reliably reruns after a language-switch recreation) so switching language mid-session actually re-populates the library, not just the UI chrome around it.
- **News is still English-only** — WHO's RSS feed and PubMed don't have Bengali content to pull from. The only way to show it in Bengali would be machine translation at fetch time, which I haven't added since auto-translating health content risks subtly changing a symptom or instruction.

- **Home screen refinement**: rebuilt with a real `Scaffold`/`TopAppBar` (was a plain custom `Row`), every card now an actual M3 `Card`/clickable `Card` instead of hand-rolled `Box`+`background`+`clickable` (proper ripple, elevation, and corner-radius tokens instead of an approximation). Fixed three real bugs along the way: the greeting was hardcoded to "Good morning" regardless of actual time; the disease preview was a naive `take(2)` off an alphabetically-sorted list (always showed the same two entries — now day-rotated, like the tip); and the news preview could surface PubMed journal titles ahead of WHO news (now `NewsRepository.observePreview` prioritizes non-journal articles, `observeAll` for the News tab is untouched). Preview counts bumped 2→3 on both News and Library sections, and the disease preview switched from horizontal chips to vertical `Card` rows matching the news rows, mainly to fill out the vertical space that was previously empty below the fold. Verified the color palette against WCAG AA computationally (worst case 4.72:1, all pairs pass) rather than assuming.
- Deliberately skipped for now: a skeleton/shimmer loading state (Room's local cache means there's rarely a visible empty-loading gap in practice, so the complexity wasn't worth it) and extending the `TopAppBar`/`Card` treatment to News/Library/Tips (this pass was scoped to Home specifically — worth doing as a follow-up for consistency).

- **Shared animated card component** (`ui/components/AnimatedCard.kt`): the Compose equivalent of a Flutter `AnimatedHealthCard` pattern the person shared — 20dp rounded corners, animated elevation (1dp default → 6dp pressed, M3 handles the animation automatically), and a scale-down-on-press micro-interaction (`0.97f`, 150ms) via `graphicsLayer` + `interactionSource`. One adaptation: the Flutter original scales *up* on mouse hover, a desktop/pointer concept; ported as scale-*down* on touch press instead, since that's the mobile-native equivalent. Two exported composables — `AnimatedCard` (generic content-slot wrapper, used for TipCard/reminder card/language card) and `AnimatedInfoCard` (the specific icon + bold title + muted subtitle shape, with optional `trailing` slot and `containerColor` override) — now used for every card across Home, News, Library, and Tips, replacing five separate hand-rolled `Box`/`Row` + `clip` + `background` + `clickable` implementations with one shared, consistently-animated component.

- **Animated bottom nav** (`ui/components/AnimatedM3BottomBar.kt`): extracted the bottom bar out of `MainActivity.kt` into its own component, adapted from a hand-rolled `AnimatedM3BottomBar` snippet — kept its spring bounce-scale on the selected tab icon (`Spring.DampingRatioMediumBouncy`, scales to 1.15x), but rewired it to route-based selection (`currentRoute.startsWith(item.route)`) to match this app's `NavController`-driven nav instead of a plain selected-index model, and kept the app's own amber `primary`/`surface` colors rather than the snippet's generic `secondaryContainer`/`surfaceContainer` roles — this custom dark theme never explicitly sets those, so they'd have resolved to auto-generated tones that clash with the rest of the palette rather than match it.
...
- **Disease dataset**: 24 entries (English + Bengali), written from general medical knowledge — not medically reviewed. A few of the new additions (rabies, snakebite envenoming) describe medical emergencies; I kept the treatment text to "get to a hospital immediately" plus WHO-endorsed general first aid, deliberately avoiding anything that could read as home treatment. Have the whole set checked before real users see it.
